/**
 * RiftLink Routing — порт для FakeTech (nRF), без FreeRTOS mutex.
 */
...
static int findBestRoute(const uint8_t* dest) {
  ...
  for (int i = 0; i < ROUTING_MAX_ROUTES; i++) {
    ...
    if (online > bestOnline ||
        (online == bestOnline && (trust > bestScore || (trust == bestScore && s_routes[i].hops < bestHops)))) {
      best = i;
      bestOnline = online;
      bestScore = trust;
...
static void addSeen(const uint8_t* originator, uint32_t reqId, const uint8_t* target) {
  ...
  for (int i = 0; i < ROUTING_MAX_SEEN; i++) {
    ...
    if ((now - s_seen[i].timestamp) >= ROUTING_SEEN_TTL_MS) {
      freeIdx = i;
      break;
    }
  }
  if (freeIdx < 0) {
    ...
    for (int i = 1; i < ROUTING_MAX_SEEN; i++) {
      if (s_seen[i].timestamp < oldest) {
        oldest = s_seen[i].timestamp;
        freeIdx = i;
      }
    }
  }
...
void requestRoute(const uint8_t* target) {
  ...
  size_t len = protocol::buildPacket(pkt, sizeof(pkt), node::getId(), protocol::BROADCAST_ID, 31, protocol::OP_ROUTE_REQ,
  ...
  if (len > 0) {
    ...
    if (!queueTxPacket(pkt, len, txSf, true, TxRequestClass::control, reasonBuf, sizeof(reasonBuf))) {
      ...
      RIFTLINK_DIAG("ROUTE", "event=ROUTE_TX_DEFER type=req to=%02X%02X cause=%s", target[0], target[1],
          reasonBuf[0] ? reasonBuf : "?");
    } else {
      RIFTLINK_DIAG("ROUTE", "event=ROUTE_TX_QUEUED type=req to=%02X%02X reqId=%u sf=%u", target[0], target[1],
          (unsigned)reqId, (unsigned)txSf);
    }
...
bool onRouteReq(const uint8_t* from, const uint8_t* payload, size_t payloadLen) {
  ...
  if (node::isForMe(target)) {
    ...
    memcpy(replyPayload + 13, from, protocol::NODE_ID_LEN);

    uint8_t pkt[protocol::PAYLOAD_OFFSET + ROUTE_PAYLOAD_LEN];
    size_t len = protocol::buildPacket(pkt, sizeof(pkt), node::getId(), sender, 31, protocol::OP_ROUTE_REPLY,
    ...
    if (len > 0) {
      ...
      if (!queueTxPacket(pkt, len, txSf, true, TxRequestClass::control, reasonBuf, sizeof(reasonBuf))) {
        queueDeferredSend(pkt, len, txSf, 60, true);
        RIFTLINK_DIAG("ROUTE", "event=ROUTE_TX_DEFER type=reply target=me to=%02X%02X cause=%s", sender[0], sender[1],
            reasonBuf[0] ? reasonBuf : "?");
      }
  ...
  uint8_t pkt[protocol::PAYLOAD_OFFSET + ROUTE_PAYLOAD_LEN];
  size_t len = protocol::buildPacket(pkt, sizeof(pkt), node::getId(), protocol::BROADCAST_ID, 31 - (hops + 1),
      protocol::OP_ROUTE_REQ, fwdPayload, ROUTE_PAYLOAD_LEN);
  if (len > 0) {
    uint8_t txSf = neighbors::rssiToSf(neighbors::getMinRssi());
    char reasonBuf[40];
    if (!queueTxPacket(pkt, len, txSf, true, TxRequestClass::control, reasonBuf, sizeof(reasonBuf))) {
      queueDeferredSend(pkt, len, txSf, 70, true);
      RIFTLINK_DIAG("ROUTE", "event=ROUTE_TX_DEFER type=req_relay hops=%u cause=%s", (unsigned)(hops + 1),
          reasonBuf[0] ? reasonBuf : "?");
    }
...
bool onRouteReply(const uint8_t* from, const uint8_t* to, const uint8_t* payload, size_t payloadLen) {
  ...
  if (node::isForMe(to)) {
    ...
    Serial.printf("[RiftLink] ROUTE_REPLY: route to %02X%02X via %02X%02X (%u hops, rssi=%d)\n", target[0], target[1],
        from[0], from[1], (unsigned)hops, rssi);
    return true;
  }
  ...
  uint8_t pkt[protocol::PAYLOAD_OFFSET + ROUTE_PAYLOAD_LEN];
  size_t len = protocol::buildPacket(pkt, sizeof(pkt), node::getId(), s_reverse[revIdx].prevHop, 31,
      protocol::OP_ROUTE_REPLY, payload, payloadLen);
  if (len > 0) {
    uint8_t txSf = neighbors::rssiToSf(neighbors::getRssiFor(s_reverse[revIdx].prevHop));
    char reasonBuf[40];
    if (!queueTxPacket(pkt, len, txSf, true, TxRequestClass::control, reasonBuf, sizeof(reasonBuf))) {
      queueDeferredSend(pkt, len, txSf, 60, true);
      RIFTLINK_DIAG("ROUTE", "event=ROUTE_TX_DEFER type=reply_relay to=%02X%02X cause=%s", s_reverse[revIdx].prevHop[0],
          s_reverse[revIdx].prevHop[1], reasonBuf[0] ? reasonBuf : "?");
    }
