...
**Клиент RiftLink (Flutter):** на Android сканирование использует актуальный стек (`androidLegacy: false` в flutter_blue_plus). Если на конкретном устройстве скан «пустой», проверьте разрешения геолокации и Bluetooth.
...
**Базовый снимок после подключения:** клиенту достаточно **одной** волны `cmd:info` — в ней уже есть маршруты и группы (`evt:routes`, `evt:groups`). Повторно слать подряд `cmd:routes` / `cmd:groups` сразу после успешного `info` не требуется (отдельные команды оставлены для точечного обновления и отладки). На всех платформах (ESP и nRF52840) начальный снимок инициирует приложение через `cmd:info`, без фоновой дублирующей волны при установлении GATT.
...
Полный `evt:node` может быть длиннее 512 байт: на BLE это **одна** логическая строка NDJSON, передаваемая **несколькими** GATT notify до завершающего `\n` (склейка буфера на клиенте — §476 ниже).  
...
От **телефона к устройству** одна логическая NDJSON-строка — **одна** GATT write в TX NUS (`6e400002`, до 512 байт полезной нагрузки с учётом `\n` в конце записи). На ESP (NimBLE) и на nRF52840 (Bluefruit) прошивка ставит каждую такую запись в очередь и разбирает JSON в главном цикле (`ble::update()`), аналогично друг другу.
...
**Прошивки nRF52840 — паритет с Heltec ESP**

- **Контракт приёмки** — поведение по **BLE** (раздел 2 этого документа) и в **эфире** (HELLO/KEY/MSG, регион, модем, телеметрия). Это то, что проверяет приложение и двухузловой сценарий.
- **USB Serial** в `faketec/main.cpp` — **вспомогательная отладка** с ПК (дубли части сценариев, дашборд на OLED/ST7789, логи). Расширение списка строк **не** является целью паритета и **не** подменяет проверку по BLE.

В Serial по-прежнему доступны базовые строки вроде `send`, `ping`, `region`, `channel`, `help` и дополнительные команды для стола; **полный перечень** — в исходниках `faketec/main.cpp` (ветка `Serial.available`). На **Heltec T114** кнопка листает локальный дашборд на экране. В фоне выполняется **retry KEY_EXCHANGE** для соседей без ключа (как на ESP). Ручной чеклист без упора на Serial: `docs/NRF52_PARITY_CHECKLIST.md`.
