...
//
// RiftLink patch: call busy callback during _waitWhileBusy (e.g. esp_task_wdt_reset)
...
GxEPD2_EPD::GxEPD2_EPD(int8_t cs, int8_t dc, int8_t rst, int8_t busy, int8_t busy_level, uint32_t busy_timeout,
  ...
  _spi_settings(4000000, MSBFIRST, SPI_MODE0), _spi(spi), _busy_callback(nullptr)
{
  _initial_write = true;
