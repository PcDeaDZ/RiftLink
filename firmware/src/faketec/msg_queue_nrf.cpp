...
// SF берём из текущего модема (паритет с ESP queueTxRequestInternal + эфирный TX), не из rssi-подсказок.
// Direct neighbors (online, known RSSI) get aggressive timeout; multi-hop keeps original.
static uint32_t getAckTimeoutMs(uint8_t /* storedTxSf */, const uint8_t* to = nullptr) {
  uint8_t txSf = radio::getMeshTxSfForQueue();
...
struct PendingMsg {
  ...
  uint8_t retries;
  uint8_t txSf;   // mesh SF (как в эфире после async_tx) — для retry/parity; таймаут ACK через getAckTimeoutMs → modem SF
  bool inUse;
...
static uint8_t s_lastSendFailReason = msg_queue::SEND_FAIL_NONE;
static uint32_t s_metricPiggyEmbedded = 0;
static uint32_t s_metricPiggyConsumed = 0;
static uint32_t s_metricSelectiveAckRx = 0;
...
static uint32_t s_ackReplayWriteIdx = 0;
...
static bool isAckReplayAndMark(const uint8_t* from, uint32_t msgId) {
  ...
  for (int i = 0; i < ACK_REPLAY_GUARD_SIZE; i++) {
    uint32_t ts = s_ackReplayGuard[i].seenAtMs;
    if (ts == 0 || (now - ts) > ACK_REPLAY_GUARD_MS) continue;
    if (s_ackReplayGuard[i].msgId == msgId &&
        s_ackReplayGuard[i].fromLo == fromLo &&
        s_ackReplayGuard[i].fromHi == fromHi) {
      return true;
    }
  }

  uint32_t idx = (s_ackReplayWriteIdx++) % ACK_REPLAY_GUARD_SIZE;
  s_ackReplayGuard[idx].fromLo = fromLo;
  s_ackReplayGuard[idx].fromHi = fromHi;
...
static inline void setLastSendFail(SendFailReason reason) {
  s_lastSendFailReason = (uint8_t)reason;
}

SendFailReason getLastSendFailReason() {
  return (SendFailReason)s_lastSendFailReason;
}
...
void init() {
  ...
  for (int i = 0; i < ACK_REPLAY_GUARD_SIZE; i++) {
    s_ackReplayGuard[i].fromLo = 0;
    s_ackReplayGuard[i].fromHi = 0;
    s_ackReplayGuard[i].msgId = 0;
    s_ackReplayGuard[i].seenAtMs = 0;
  }
  s_ackReplayWriteIdx = 0;
...
static bool buildUnicastPlainV3(const uint8_t* to, uint32_t msgId, const char* text,
  ...
  if (piggyCount > 0) {
    s_metricPiggyConsumed += piggyCount;
    s_metricPiggyEmbedded += 1;
  }
...
bool enqueue(const uint8_t* to, const char* text, uint8_t ttlMinutes,
  ...
  size_t textLen = strlen(text);
  if (textLen == 0) { /* mq_unlock */ setLastSendFail(SEND_FAIL_EMPTY); return false; }  // пустые — не отправлять
  ...
  if (isUnicast) {
    ...
    for (int encTry = 0; encTry < MSG_ENCRYPT_RETRY_COUNT; encTry++) {
      ...
      if (encTry + 1 < MSG_ENCRYPT_RETRY_COUNT) {
        delay(MSG_ENCRYPT_RETRY_DELAY_MS);
      }
    }
    if (!encOk) {
    ...
    const uint8_t txSf = radio::getMeshTxSfForQueue();
    slot->txSf = txSf;
    ...
    if (len > 0) {
      ...
      if (deferredCopies >= 2) queueDeferredSend(pkt, len, sf, 220 + (uint32_t)random(130));
      if (deferredCopies >= 3) queueDeferredSend(pkt, len, sf, 440 + (uint32_t)random(120));
      if (deferredCopies >= 4) queueDeferredSend(pkt, len, sf, 800 + (uint32_t)random(150));
      return true;
    }
    return false;
  }
...
bool enqueueGroup(uint32_t groupId, const char* text) {
  ...
  size_t textLen = strlen(text);
  if (textLen == 0) { /* mq_unlock */ return false; }  // пустые — не отправлять
  constexpr size_t maxPlain = MAX_SINGLE_PLAIN - GROUP_ID_LEN - MSG_ID_LEN;
  if (textLen > maxPlain) { /* mq_unlock */ return false; }
...
bool registerPendingFromFusion(const uint8_t* to, uint32_t msgId, const uint8_t* pkt, size_t pktLen, uint8_t /* txSf */) {
  ...
  slot->lastSendTime = millis();
  slot->retries = 0;
  slot->txSf = radio::getMeshTxSfForQueue();
  slot->inUse = true;
  slot->triggerSend = false;
  slot->lastAction = -1;
  slot->critical = false;
...
void update() {
  ...
  for (int i = 0; i < MAX_PENDING; i++) {
    ...
    if (p->triggerSend) {
      p->triggerSend = false;
      uint32_t jitter = 50 + (uint32_t)random(100);
  ...
  if ((now - s_lastMetricLogMs) >= 15000) {
    ...
    RIFTLINK_DIAG("V3", "event=V3_METRICS piggy_embedded=%lu piggy_consumed=%lu selective_ack_rx=%lu",
        (unsigned long)s_metricPiggyEmbedded,
        (unsigned long)s_metricPiggyConsumed,
        (unsigned long)s_metricSelectiveAckRx);
  }
  /* mq_unlock */
}
