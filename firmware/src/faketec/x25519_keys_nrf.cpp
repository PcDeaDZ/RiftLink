...
void init() {
...

  {
    ...
    if (riftlink_kv::getBlob(NVS_KEY_X25519_PUB, s_pubKey, &len) && len == X25519_PUBKEY_LEN) {
      len = crypto_box_SECRETKEYBYTES;
      if (riftlink_kv::getBlob(NVS_KEY_X25519_SEC, s_secKey, &len) && len == crypto_box_SECRETKEYBYTES) {
        s_inited = true;
  ...
  /* esphome/libsodium без crypto_box — пара ключей как в libsodium: sk случайный, pk = scalarmult_base(sk) */
  randombytes_buf(s_secKey, crypto_scalarmult_SCALARBYTES);
  if (crypto_scalarmult_base(s_pubKey, s_secKey) != 0) {
    memset(s_pubKey, 0, sizeof(s_pubKey));
    memset(s_secKey, 0, sizeof(s_secKey));
    return;
  }

  (void)riftlink_kv::setBlob(NVS_KEY_X25519_PUB, s_pubKey, X25519_PUBKEY_LEN);
  (void)riftlink_kv::setBlob(NVS_KEY_X25519_SEC, s_secKey, crypto_box_SECRETKEYBYTES);
  memset(s_peers, 0, sizeof(s_peers));
  memset(s_peerKeyCache, 0, sizeof(s_peerKeyCache));
  publishPeerKeyCacheLocked();
  for (int i = 0; i < 4; i++) {
...
void onKeyExchange(const uint8_t* peerId, const uint8_t* theirPubKey) {
  ...
  /* Как libsodium crypto_box_beforenm: scalarmult → crypto_core_hsalsa20 (паритет с ESP). */
  unsigned char s[32];
  if (crypto_scalarmult(s, s_secKey, theirPubKey) != 0) {
    xSemaphoreGive(s_mutex);
    RIFTLINK_DIAG("KEY", "event=KEY_STORE_FAIL cause=scalarmult_failed peer=%02X%02X pub0=%02X pub1=%02X",
        peerId[0], peerId[1], theirPubKey[0], theirPubKey[1]);
    RIFTLINK_LOG_ERR("[RiftLink] X25519 crypto_scalarmult FAILED peer=%02X%02X pub32=%02X%02X...\n",
        peerId[0], peerId[1], theirPubKey[0], theirPubKey[1]);
  ...
  static const unsigned char hsalsaIn[16] = {0};
  static const unsigned char sigma[16] = {
      'e', 'x', 'p', 'a', 'n', 'd', ' ', '3', '2', '-', 'b', 'y', 't', 'e', ' ', 'k'};
  if (crypto_core_hsalsa20(s_peers[idx].sharedKey, hsalsaIn, s, sigma) != 0) {
    xSemaphoreGive(s_mutex);
    RIFTLINK_DIAG("KEY", "event=KEY_STORE_FAIL cause=hsalsa20_failed peer=%02X%02X", peerId[0], peerId[1]);
    RIFTLINK_LOG_ERR("[RiftLink] X25519 crypto_core_hsalsa20 FAILED peer=%02X%02X\n", peerId[0], peerId[1]);
    sendKeyExchange(peerId, true, false, "key_store_hsalsa_fail");
    return;
  }
...
void sendKeyExchange(const uint8_t* peerId, bool forceSend, bool hadKeyBefore, const char* reason) {
  ...
  if (len > 0) {
    ...
    if (primaryPath) {
      ...
      uint32_t secondMs = firstMs + KEY_STALE_SECOND_CHANCE_MS + ((uint32_t)random() % 40u);
