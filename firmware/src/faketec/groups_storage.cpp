/**
 * Groups V2 на storage:: (InternalFS), паритет с firmware/src/groups/groups.cpp (NVS).
 */
...
static void persistV2() {
  if (!storage::setLargeBlob(NVS_KEY_GROUPS_V2, (const uint8_t*)s_groupsV2, sizeof(s_groupsV2))) return;
  (void)storage::setI8(NVS_KEY_GROUPS_V2_CNT, (int8_t)s_countV2);
}
...
void init() {
  ...
  const bool largeOk = storage::getLargeBlob(NVS_KEY_GROUPS_V2, (uint8_t*)s_groupsV2, &groupsV2Len) &&
      groupsV2Len == sizeof(s_groupsV2);
  if (!largeOk) {
    memset(s_groupsV2, 0, sizeof(s_groupsV2));
  } else {
    /* gr2c имеет смысл только вместе с gr2s; иначе RAM/FS рассинхрон (малый ключ в RAM, большой блоб только во flash). */
    int8_t groupsV2Cnt = 0;
    if (storage::getI8(NVS_KEY_GROUPS_V2_CNT, &groupsV2Cnt) && groupsV2Cnt >= 0) {
...
bool upsertGroupV2(const char* groupUid, uint32_t channelId32, const char* groupTag, const char* canonicalName,
    const uint8_t* groupKey32, uint16_t keyVersion, GroupRole myRole, uint32_t revocationEpoch) {
...
bool getGroupV2(const char* groupUid, uint32_t* outChannelId32, char* outGroupTag, size_t outGroupTagLen,
    char* outCanonicalName, size_t outCanonicalNameLen, uint16_t* outKeyVersion, GroupRole* outRole,
    uint32_t* outRevocationEpoch, bool* outAckApplied) {
  int idx = findV2Index(groupUid);
  if (idx < 0) return false;
...
bool getV2At(int index, char* outGroupUid, size_t outGroupUidLen, uint32_t* outChannelId32, char* outGroupTag,
    size_t outGroupTagLen, char* outCanonicalName, size_t outCanonicalNameLen, uint16_t* outKeyVersion,
    GroupRole* outRole, uint32_t* outRevocationEpoch, bool* outAckApplied) {
  ...
  if (outCanonicalName && outCanonicalNameLen > 0)
    copySafe(outCanonicalName, outCanonicalNameLen, s_groupsV2[index].canonicalName);
