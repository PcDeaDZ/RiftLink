...
//
// RiftLink patch: added setBusyCallback for esp_task_wdt_reset during long BUSY waits
...
{
  public:
    // Callback type: called during _waitWhileBusy to e.g. reset watchdog
    typedef void (*busy_callback_t)(const void*);
    ...
    // Set callback invoked during BUSY wait (e.g. esp_task_wdt_reset on ESP32)
    void setBusyCallback(busy_callback_t cb) { _busy_callback = cb; }
  ...
  protected:
    ...
    SPIClass &_spi;
    busy_callback_t _busy_callback;
};
