...
**Прошивки nRF52840 (USB Serial):** в `faketec/main.cpp` поддерживаются `send`, `ping` (как на ESP: при ошибке сборки пакета — `PING failed`), `node` или `id` (полный Node ID hex и никнейм), **`region`** без аргументов — текущий код региона, частота и номер канала; **`region <code>`** — смена региона как на ESP, `channel`, `nickname`, `route`, `lang`, `memdiag` (куча SoftDevice + задачи FreeRTOS), **`bat`** или **`battery`** — напряжение батареи (мВ), при наличии оценка % и флаг charging (на FakeTech без АЦП батареи может быть 0 мВ), `neighbors` или `nb` (список соседей, RSSI, ключ, **bat_mv** из последней телеметрии соседа), `gps` / `gps on` / `gps off` / `gps pins <rx> <tx> [en]` (на nRF без UART GNSS — в основном KV и синхронизация с телефоном), `selftest` или `test`, `sf` или `radio` (дополнительно **bw**/**cr** модема, как в логе модема), `modemscan` / `modemscan quick` / `modemscan full` (скан эфира, `full` дольше), `version` или `ver` (версия прошивки и дата сборки), `uptime` (секунды с запуска), `powersave` / `powersave on` / `powersave off` (на nRF нет режима энергосбережения как на ESP — явные строки в Serial и в BLE — `powersave_unsupported`), `espnow` / `espnow …` (ответ: не применимо, без Wi‑Fi), `help`. В фоне, как на Heltec ESP, периодически выполняется **retry KEY_EXCHANGE** для соседей без ключа (лог `KEY_RETRY_*` при включённой диагностике). Чеклист двух узлов: `docs/NRF52_PARITY_CHECKLIST.md`.
