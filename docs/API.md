...
Устаревший монолитный `evt: "info"` (один JSON со всеми полями) может встречаться в старых сборках; актуальные прошивки (Heltec, FakeTech BLE) используют волну выше.
...
**FakeTech (nRF52840 / NiceNano):** на плате **нет Wi‑Fi** (только BLE + LoRa). OTA по Wi‑Fi как на ESP **невозможен**; прошивка через USB DFU (копирование `firmware.zip` на диск `NICENANO` в режиме загрузчика). Команды `{"cmd":"ota"}`, `{"cmd":"radioMode","mode":"wifi"}`, `{"cmd":"wifi",...}` на FakeTech возвращают `evt: error` с пояснением. В снимке узла (`evt: node`) для FakeTech всегда `radioMode: "ble"`, `wifiConnected: false`.
